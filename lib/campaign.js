...
} from './real-ai-guard.js'

export { AGENTS, AGENT_ORDER } from './campaign-agents.js'

export const CAMPAIGN_MODEL = process.env.CAMPAIGN_MODEL || 'claude-sonnet-4-20250514'
...
export async function streamCampaign(
  ...
  send,
  { createClient = () => new Anthropic(), requireRealUsage = true } = {}
) {
  const client = createClient()
  const platformList = platforms || ['twitter', 'tiktok']

  const guardUsage = (usage, agent) => {
    if (requireRealUsage) assertRealAgentUsage(usage, agent)
  }
  const guardContent = (data, agent) => {
    if (requireRealUsage) assertNoMockContent(data, agent)
  }
  ...
  guardUsage(marketResult.usage, 'marketAnalyst')
  guardContent(marketData, 'marketAnalyst')
  ...
  guardUsage(contentResult.usage, 'contentWriter')
  guardContent(contentData, 'contentWriter')
  ...
  guardUsage(growthResult.usage, 'growthOptimizer')
  guardContent(growthData, 'growthOptimizer')
  ...
  if (requireRealUsage) assertRealCampaignUsage(result.usage)
  send({
    type: 'agent_done',
